...
> so you might need to disable that in [`@nxss/auth`](./packages/auth/src/index.ts)
...
> In this template, we use `@nxss` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@nxss` to something like `@my-company` or `@project-name`.
